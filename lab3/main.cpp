...
int main()
{
	...
	cout << s1 << endl;;
	s1.laduj();
	cout << s1 << endl;		//wylądował

	s1.przygotuj_do_startu(22, 49, "Kraków");
	cout << s1 << endl;;
	s1.start();
	cout << s1 << endl;;
	...
	cout << "s1 startował " << s1.pobierz_starty() << " razy" << endl;	//2
	cout << "s2 startował " << s2.pobierz_starty() << " razy" << endl;	//0
	cout << "s3 startował " << s3.pobierz_starty() << " razy" << endl;	//1
	cout << "Wszystkich startów " << s1.start() << endl;	//3: 2 razy s1, 1 raz s3
	cout << "Samolotów w powietrzu " << Samolot::pobierz_liczba_w_powietrzu() << endl;	//2: s1, s3


		cout << endl << "ETAP 2 ----------------------------------------" << endl << endl;
	
		Lotnisko lot_1;
		lot_1 += s1;	//leci i wylądował
		lot_1 += s2;
		lot_1 += s3;	//leci i wylądował
		Samolot s4;
		lot_1 += s4;
		cout << lot_1 << endl;
	
		s4.przygotuj_do_startu(0,44,"Wrocław");
		s1.przygotuj_do_startu(14, 33, "Katowice");
		s2.przygotuj_do_startu(0, 21, "Gdańsk");
		cout << lot_1 << endl;
		...
		if(lot_1.zezwol_na_start(s1))
			...
			cout << s1 << ", więc nie może wystartować ponownie" << endl;
	
		cout << "Próba ponownego wystartowania samolotu s1 :" << endl;
		if (lot_1.zezwol_na_start(s1))
			cout << s1 << " z lotniska nr 1" << endl;
		else
			cout << s1 << " więc nie może wystartować ponownie" << endl;
		...
		cout << lot_1 << endl;
		//--------------------------------------------------------
		cout << endl << "Przygotowanie samolotów na lotnisko 2:" << endl;
		Samolot s5, s6, s7, s8, s9, s10, s11;
		s5.przygotuj_do_startu(0, 77, "Koluszki");
		s5.start();
		s6.przygotuj_do_startu(0, 45, "Modlin");
		s6.start();
		s7.przygotuj_do_startu(0, 73, "Rzeszów");
		s7.start();
		s8.przygotuj_do_startu(0, 99, "Suwałki");
		s8.start();
		s9.przygotuj_do_startu(0, 12, "Poznań");
		s9.start();
		s9.przygotuj_do_startu(0, 12, "Poznań");
		cout << s5 << endl;
		cout << s6 << endl;
		...
		cout << s1 << endl;
		cout << "s1 wylatywał " << s1.pobierz_starty() << " razy" << endl;
		...
		cout << s4 << endl;
		cout << "s4 wylatywał " << s4.pobierz_starty() << " razy" << endl;
	
		cout << endl << "Samoloty przed naprawą: ";
		cout << lot_1 << endl;

		/*co trzeba dopisać do klas, by poniższy fragment był poprawny? */
